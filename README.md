...
**Pattern:** Sonnet main agent (Victor) handles all coding with [ponytail](https://github.com/DietrichGebert/ponytail) minimal-code discipline. Opus advisory subagents fire only when needed — planner on complex tasks, reviewer after every code change.

## Agents

| Agent | Model | Visible | Role |
|-------|-------|---------|------|
| `Victor — Coder` | Claude Sonnet 4.6 | ✅ | Main coding agent — asks questions, ponytail discipline, per-project memory |
| `Advisor — Plan` | Claude Opus 4.8 | ❌ subagent only | Deep planner for complex/ambiguous tasks |
| `Advisor — Review` | Claude Opus 4.8 | ❌ subagent only | Mandatory post-change reviewer (ponytail lens + OWASP) |
